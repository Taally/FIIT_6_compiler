...
Методы `Get_e_gen` и `Get_e_kill` отвечают соответственно за получение множеств `e_gen` и `e_kill` для каждого базового блока программы. Они работают в точности так, как было описано равнее в теоретической части.
...
При инициализации объектов сначала задается оператор сбора. В случае доступных выражений таковым является пересечение.
